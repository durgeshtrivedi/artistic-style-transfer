...
content_image_path = 'images/pslogo512.jpg'
...
style_image_path = 'images/styles/kandinsky_circle.jpg'
...
for i in range(iterations):
    ...
    imgout=x
    imgout = imgout.reshape((height, width, 3))
    imgout = imgout[:, :, ::-1]
    imgout[:, :, 0] += 103.939
    imgout[:, :, 1] += 116.779
    imgout[:, :, 2] += 123.68
    imgout = np.clip(imgout, 0, 255).astype('uint8')
    Image.fromarray(imgout)
    imsave('images/output/' + str(i)+".jpg", imgout)
...
Image.fromarray(x)
imsave('images/output/'+"final.jpg", x)
